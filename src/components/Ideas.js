...
render() {
  let sortedIdeas 
    if (this.state.isSorted === true) {
      ...
      sortedIdeas = this.props.ideas.concat().sort((i, j) => i.body.localeCompare(j.body, {
        sensitivity: 'base'
    }));
    } else {
      // Ideas sorted by creation date
      sortedIdeas = this.props.ideas.concat().sort((i, j) => j.created_at.localeCompare(i.created_at, {                
        sensitivity: 'base'
    }));
  }
 ...
 const renderIdeas = sortedIdeas.map(idea =>
    <div className="bucketlist card" key={idea.id}>
      ...
      <div className="card-content-small">
        ...
        <button class="waves-effect btn-small red darken-4"
         onClick={() => this.props.deleteIdea(idea.id)}>click to complete
        </button>
      </div>
    </div>
  )
