...
export const addIdea = idea => { 
  ...
  return dispatch => {  // dispatch an action to state that we are about to make a request to our API
    return IdeaAPI.createIdea(idea) // then make a request to the API -->  GO TO: API createIdea(idea)
      .then(idea => {  // We do not hit our then() function until the response is received
        ...
        dispatch(addIdeaAction(idea)); // when the promise resolves, we dispatch another action with 'type' --> GO TO: addIdeaAction on line 12
      })
    .catch(error => console.log(error));
  }
}
